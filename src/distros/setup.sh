...
# == installing packages == #
# Manjaro
sudo pacman -S $(cat src/distros/distro-pkg/manjaro) --noconfirm
# Solus
sudo eopkg install $(cat src/distros/distro-pkg/solus) -y 
#Fedora
sudo dnf install $(cat src/distros/distro-pkg/fedora) -y --allowerasing --skip-broken
# Ubuntu
sudo apt-get install $(cat src/distros/distro-pkg/ubuntu) -y 
